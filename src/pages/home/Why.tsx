...
const whyContent = [
   {
      ...
      description:
         'Are you fascinated by the latest gadgets, cutting-edge software, or emerging trends in the world of technology? If so, Chatter is the perfect place for you.',
   },
   {
      id: 2,
      icon: <GiRingingBell />,
...
export const Why = (): React.JSX.Element => {
   return (
      <div className="why-container mt-20 mobileXL:mt-12 pb-8">
         <Typography variant={2} className=" text-4xl mobileXL:text-2xl font-bold mb-8 text-center">
         ...
         <div className=" flex flex-wrap justify-center items-center">
            {whyContent.map((item) => {
               ...
               return <WhyCard key={item.id} title={title} description={description} icon={icon} />;
            })}
         </div>
      </div>
   );
